...
- (void)reloadData{
    ...
    for (int section = 0; section < [_emojiDelegate countOfEmojiPageSection]; section++){
        self.backgroundColor = [UIColor whiteColor];//自己添加
...
- (void)showEmojiPreview{
    [self getEmojiFromCurrentLocation:^(CGPoint emojiCenterInSelf, NSString *emoji) {
        if (emoji){
            ...
            [_emojiPreview setCenter:CGPointMake(emojiCenterInSelf.x,emojiCenterInSelf.y-(CGRectGetHeight(_emojiPreview.frame)-_emojiHeight)/2 + 15) ];
        }
        else{
            _emojiPreview.hidden = YES;
        }
...
- (void)getEmojiInBounds:(CGRect)bounds
    ...
    NSInteger row = (NSInteger)((location.y - 15 * kScaleX)/_emojiHeight) + 1;
...
//    [self updateTitlePosition];
    if ([_emojiDelegate respondsToSelector:@selector(didScrollToSection:)]){
        [_emojiDelegate didScrollToSection:[self sectionForLocation:CGPointMake(scrollView.contentOffset.x, kSTEPTitleHeight+15 * kScaleX)]];
    }
}
