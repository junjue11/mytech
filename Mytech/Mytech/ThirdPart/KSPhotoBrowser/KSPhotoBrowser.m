...
static const CGFloat kPageControlBottomSpacing = 32;
...
// 牛数埋点的相关属性
@property (nonatomic, strong) NSString *current_page_id;
@property (nonatomic, strong) NSString *source_page_id;
...
- (instancetype)initWithPhotoItems:(NSArray<KSPhotoItem *> *)photoItems selectedIndex:(NSUInteger)selectedIndex {
    ...
    if (self) {
        ...
        self.source_page_id = @""; //[UIViewController lv_getSourcePageId];
        self.current_page_id = @"pic_preview_dynamic_page";
        ...
        _imageManager = [[KSSDImageManager alloc] init];
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    // 分享
    UIButton *shareBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.width - 61 * kScaleX, self.view.height - 53 * kScaleX, 36 * kScaleX, 36 * kScaleX)];
    [shareBtn setImage:XNImage(@"chat_look_image_share") forState:UIControlStateNormal];
    [self.view addSubview:shareBtn];
    [shareBtn addTarget:self action:@selector(sharePhoto) forControlEvents:UIControlEventTouchUpInside];
    
    // 下载
    UIButton *downloadBtn = [[UIButton alloc] initWithFrame:CGRectMake(shareBtn.left - 36 * kScaleX - 20 * kScaleX, shareBtn.top, 36 * kScaleX, 36 * kScaleX)];
    [downloadBtn setImage:XNImage(@"chat_look_image_download") forState:UIControlStateNormal];
    [self.view addSubview:downloadBtn];
    [downloadBtn addTarget:self action:@selector(downloadPhoto) forControlEvents:UIControlEventTouchUpInside];
    
    if (_pageindicatorStyle == KSPhotoBrowserPageIndicatorStyleUser) {
        _userImageView = [[UIImageView alloc] initWithFrame:CGRectMake(17, self.view.bounds.size.height - 49 * kScaleX, 28 * kScaleX, 28 * kScaleX)];
        _userImageView.layer.cornerRadius = 14 * kScaleX;
        _userImageView.layer.masksToBounds = YES;
        [self.view addSubview:_userImageView];
        
        _userLabel = [[UILabel alloc] initWithFrame:CGRectMake(_userImageView.right + 10 * kScaleX, 0, 200 * kScaleX, 19 * kScaleX)];
        _userLabel.textColor = [UIColor whiteColor];
//        _userLabel.font = [UIFont regularFontWithSize:14]; // TODO
        _userLabel.centerY = _userImageView.centerY;
        [self configPageLabelWithPage:_currentPage];
        [self.view addSubview:_userLabel];
    } else if (_pageindicatorStyle == KSPhotoBrowserPageIndicatorStyleDot) {
...
//        _pageLabel = [[UILabel alloc] initWithFrame:CGRectMake(12, self.view.bounds.size.height - kPageControlBottomSpacing - [XNScreenTool getHIBarHeight], 200, kPageControlHeight)];
        _pageLabel.textColor = [UIColor whiteColor];
//        _pageLabel.font = [UIFont regularFontWithSize:14];// TODO
//        [self configPageLabelWithPage:_currentPage];
...
/** 保存到相册*/
- (void)downloadPhoto{
    KSPhotoItem *item = _photoItems[_currentPage];
    if (!(item && item.imageUrl)) {
//        [XNProgressHUD showMessage:@"保存失败"];
        return;
    }
    
    [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:item.imageUrl
                                                          options:SDWebImageDownloaderUseNSURLCache
                                                         progress:nil
                                                        completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished) {
                                                            if (error || !image) {
//                                                                [XNProgressHUD showMessage:@"保存失败"];// TODO
                                                            }else{
...
//    [XNProgressHUD showMessage:msg];// TODO
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([_imageManager imageFromMemoryForURL:item.imageUrl]) {
        [photoView updateDownloadImage:YES];
        [self configPhotoView:photoView withItem:item];
    } else {
        [photoView updateDownloadImage:NO];
...
//            self.view.backgroundColor = COLOR_BLACK_1;// TODO
...
- (void)configPageLabelWithPage:(NSUInteger)page {
    if (_pageindicatorStyle == KSPhotoBrowserPageIndicatorStyleText) {
        _pageLabel.text = [NSString stringWithFormat:@"%lu/%lu", page+1, _photoItems.count];
    } else if (_pageindicatorStyle == KSPhotoBrowserPageIndicatorStyleUser) {
        KSPhotoItem *item = [_photoItems objectAtIndex:_currentPage];
        if ([item.extend isKindOfClass:[NSDictionary class]]) {
            NSURL *headUrl = item.extend[@"headUrl"];
            NSString *name = item.extend[@"name"];
//            [_userImageView yy_setImageWithURL:headUrl placeholder:nil];
            [_userImageView setImageWithURL:headUrl placeholder:nil];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (_dyId && _dyId.length > 0) {
//        [NiuDataStatistics clickWtihCode:@"slide_pic_preview_dynamic_click" eventName:@"滑动动态图片预览" priority:XNEventPriority_InBatchesUpload extendParameter:@{@"source_page_id" : NSNULLText(_source_page_id), @"current_page_id" : NSNULLText(_current_page_id), @"content_id" : _dyId, @"pic_number" : [NSString stringWithFormat:@"%zi", _photoItems.count]}];
    }
}
