...
- (void)wm_addMenuView {
    ...
    menuView.normalFont = self.normalFont;
    menuView.selectedFont = self.selectedFont;
...
- (CGFloat)wm_calculateItemWithAtIndex:(NSInteger)index {
    NSString *title = [self titleAtIndex:index];
    if (_selectedFont && _normalFont) {
        UIFont *titleFont = _selectedFont;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (_startDragging) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(pageController:slideMenuAtProgress:)]) {
            [self.delegate pageController:self slideMenuAtProgress:rate];
        }
    }
...
- (void)menuView:(WMMenuView *)menu didSelesctedIndex:(NSInteger)index currentIndex:(NSInteger)currentIndex {
    ...
    if (_clickMenuItemBlock) {
        _clickMenuItemBlock(index);
    }
}
...
- (CGFloat)menuView:(WMMenuView *)menu itemMarginAtIndex:(NSInteger)index {
    ...
    if (_fixedLeft && (index == 0 || index == self.childControllersCount)) {
        return 3;
    }
