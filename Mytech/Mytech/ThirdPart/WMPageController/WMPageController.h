...
/**
 * 新增代理方法，监测pageController位移
 @param pageController The parent controller (WMPageController)
 @param rate 进度
 */
- (void)pageController:(WMPageController *)pageController slideMenuAtProgress:(CGFloat)rate;
 ...
 * 点击顶部MenuView对应事件
 */
@property (nonatomic, copy) void(^clickMenuItemBlock)(NSInteger index);
...
@property (nonatomic, strong) UIFont *normalFont;   ///> Normal状态的字体
@property (nonatomic, strong) UIFont *selectedFont; ///> Selected状态的字体
 ...
 * 是否从左侧固定
 */
@property (nonatomic, assign) BOOL fixedLeft;
